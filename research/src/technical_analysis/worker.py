...
import pandas as pd
import polars as pl
from signals.technical_analysis import DEFAULT_LOOKBACK_DAYS, technical_analysis_frame

DEFAULT_HORIZONS = (5, 20)
DEFAULT_THRESHOLDS = (0.15, 0.30, 0.50)
DEFAULT_TEST_FRACTION = 0.30
MIN_PRECISION_FOR_WEIGHT = 0.55
MIN_WORKER_LOOKBACK_DAYS = 40
FEATURE_COLUMNS = (
...
class TechnicalAnalysisWorkerLoader(Protocol):
...
class TechnicalAnalysisWorkerConfig:
    start: date
    end: date
    tickers: tuple[str, ...]
...
@dataclass(frozen=True)
class TechnicalAnalysisWorkerResult:
...
def run_technical_analysis_worker(
    *,
    config: TechnicalAnalysisWorkerConfig,
    loader: TechnicalAnalysisWorkerLoader,
    output_root: Path,
) -> TechnicalAnalysisWorkerResult:
    """Write technical-analysis feature, IC, threshold, and guidance artifacts."""
    ...
    return TechnicalAnalysisWorkerResult(
        config,
        features,
        ic,
        threshold_sweep,
...
def build_feature_history(
    ...
    for as_of in _evaluation_dates(config):
        frame = technical_analysis_frame(as_of, universe, loader, config.lookback_days)
        if frame.empty:
            continue
...
def calibration_to_markdown(calibration: dict[str, object]) -> str:
    lines = [
        "# Technical Analysis Worker Calibration",
        "",
        ...
        "## Runtime Guidance",
        "",
        "| Feature | Recommendation | Weight | Threshold | Horizon | "
    ...
    for feature, item in sorted(guidance.items()):
        if not isinstance(item, dict):
            raise TypeError("guidance rows must be objects")
        lines.append(
            "| "
            f"{feature} | {item['recommendation']} | {_fmt(item['suggested_weight'])} | "
    ...
    for feature in FEATURE_COLUMNS:
        for horizon in config.horizons:
            for threshold in config.thresholds:
                selected = merged[pd.to_numeric(merged[feature], errors="coerce") >= threshold]
                for window, frame in (
                    ("train", selected[selected["date"] <= split_date]),
                    ("test", selected[selected["date"] > split_date]),
                    ("all", selected),
                ):
                    rows.append(_threshold_row(feature, horizon, threshold, window, frame))
    return pd.DataFrame(rows)
...
def _threshold_row(
    ...
    return {
        ...
        "precision_positive": float((returns > 0.0).mean()) if selected_count else float("nan"),
...
def _calibration(
    ...
    verdict = "technical_weight_eligible" if eligible else "context_only_until_more_coverage"
    return {
        "schema_version": "0.1.0",
        "worker": "technical_analysis_worker",
...
def _default_guidance() -> dict[str, object]:
    return {
...
def _recommendation(config: TechnicalAnalysisWorkerConfig, test: pd.Series) -> str:
...
def _suggested_weight(test: pd.Series) -> float:
    ...
    return round(min(0.65, max(0.15, precision_edge * 3.0)), 3)
...
def _write_outputs(
    ...
    *,
    ...
    outputs = {
        "technical-analysis-features.csv": features,
        "technical-analysis-ic.csv": ic,
        "technical-analysis-threshold-sweep.csv": threshold_sweep,
    }
    for name, frame in outputs.items():
        frame.to_csv(output_root / name, index=False)
    (output_root / "technical-analysis-calibration.json").write_text(
        json.dumps(calibration, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    (output_root / "technical-analysis-calibration.md").write_text(
        calibration_to_markdown(calibration),
        encoding="utf-8",
    )
    return (
        *outputs.keys(),
        "technical-analysis-calibration.json",
        "technical-analysis-calibration.md",
    )
...
def _evaluation_dates(config: TechnicalAnalysisWorkerConfig) -> list[date]:
    return [
        item.date()
...
def _config_json(config: TechnicalAnalysisWorkerConfig) -> dict[str, object]:
    data = asdict(config)
...
def _rationale(verdict: str) -> str:
    if verdict == "technical_weight_eligible":
        return "At least one technical-analysis feature passed train and holdout checks."
    return (
        "Technical-analysis features are available, but the worker did not find "
...
def _validate_config(config: TechnicalAnalysisWorkerConfig) -> None:
    ...
    if config.lookback_days < MIN_WORKER_LOOKBACK_DAYS:
        raise ValueError(f"lookback_days must be >= {MIN_WORKER_LOOKBACK_DAYS}")
