...
| **Technical Analysis Worker** | Score chart quality and validate technical features before runtime weight changes. | PIT daily OHLCV, Massive/Polygon stock prints when available, train/test threshold sweeps. | Trend, momentum, volume, relative-strength, candle-regime, chart-pattern features, IC checks, runtime guidance. |
| **Market-Flow Analysis Worker** | Tune and validate Massive/Polygon stock-trade pressure features before runtime weight changes. | PIT `stock_trades`, daily prices, train/test threshold sweeps. | Feature panels, IC checks, threshold calibration, runtime guidance, and audit artifacts. |
