...
def test_shared_dashboard_data_health_humanizes_monitor_seconds() -> None:
    health = shared_module.dashboard_data_health(
        ...
        data_load_status={
            ...
            "cycle_id": "live-ready-20260516",
            "health_monitor": {
                "status_label": "Health Monitor Stale",
                "status_class": "block",
                "live": False,
                "reliable": False,
                "row_count": 7,
...
def test_shared_dashboard_data_health_labels_stale_monitor_as_health_check_stale() -> None:
    health = shared_module.dashboard_data_health(
        "Command dashboard",
        data_load_status={
            "overall_percent": 97,
    ...
    assert health["status_label"] == "Health Check Stale"
    assert "health check is stale" in str(health["headline"]).lower()
    assert "stale or blocked data" not in str(health["headline"]).lower()
...
def test_full_live_readiness_view_uses_plain_readiness_and_tooltip_labels() -> None:
    view = command_module.full_live_readiness_view(
        {
            ...
            "coverage": {
                ...
                "source_headline": "Critical stale source: Daily Market Bars",
    ...
    assert view["command_map"]["freshness"]["value"] == "Health check stale"
...
def test_full_live_readiness_view_never_marks_gated_review_ready() -> None:
    ...
    assert "Daily bars are stale" in view["blocking_reason_label"]
